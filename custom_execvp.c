#include "shell.h"
/**
 * _execvp - custom exevp function using execve as base
 *
...
int _execvp(char *file, char *argv[])
{
	...
	dir = strtok(path_copy, ":"); /* tokenize the path value*/
	while (dir != NULL)
	{
...
path_t *get_path(void)
{
	...
	/*path_t *temp; , no need for malloc as will copy from *head */
	path_t  *head = NULL;
	path_t *new_node;
	path_t *tail = NULL;
...
/**
 * matched_path - function to search if command is executable
 * @head : linked list input consisting of path variable
...
char *matched_path(path_t *head, char *cmd)
{
	...
	while (current)
	{
		full_path = (char *)malloc(strlen(current->path) + strlen(cmd) + 2);
		/* +2 for'/' &  \0' */
		if (full_path == NULL)
		{
